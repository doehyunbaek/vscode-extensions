...
| `Pi: Open in New Window`      | —                | Open pi in a new VS Code window                                                          |
| `Pi: Focus on Packages View`  | —                | VS Code-generated command that focuses the Packages sidebar view                         |
| `Pi: Focus on Sessions View`  | —                | Focus the Sessions sidebar view                                                          |
...
- **Sessions view** — Read `~/.pi/agent/sessions`, group recent `.jsonl` sessions by session directory/workspace, show timestamps, size, and message previews, and open or reveal session files. Progress/debug logs are written to the **Pi** output channel (`View: Show Output` → `Pi`).
