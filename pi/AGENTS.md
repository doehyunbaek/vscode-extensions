...
- **Sessions sidebar view** reads `~/.pi/agent/sessions`, groups recent JSONL sessions by session directory/workspace, lists workspace/timestamps/message previews, lets users open or reveal session files, and logs progress/debug output to the `Pi` output channel
...
- `Pi: Focus on Packages View` — VS Code-generated command for the contributed Packages sidebar view
- `Pi: Focus on Sessions View` — Focuses the contributed Sessions sidebar view
...
- Pi binary auto-detected from workspace `node_modules/.bin`, common global paths (`~/.bun/bin/pi`, `~/.local/bin/pi`, etc.), nvm-managed Node versions (`~/.nvm/versions/node/*/bin/pi`, newest first), and `PATH`, or configurable via `pi-vscode.path` setting
