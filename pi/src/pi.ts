...
export function createPiEnvironment(
  ...
  const pathWithPiBinary = createPathWithPiBinary(piPath);
  if (pathWithPiBinary) env[getPathEnvKey()] = pathWithPiBinary;

  return Object.keys(env).length > 0 ? env : undefined;
}

function createPathWithPiBinary(piPath: string | undefined): string | undefined {
  if (!piPath || !isAbsolute(piPath)) return undefined;
  ...
  const alreadyIncluded =
    process.platform === "win32"
      ? paths.some((entry) => entry.toLowerCase() === piDir.toLowerCase())
      : paths.includes(piDir);
  if (alreadyIncluded) return currentPath;

  return currentPath ? `${piDir}${delimiter}${currentPath}` : piDir;
}

function getPathEnvKey(): "PATH" | "Path" {
  return process.platform === "win32" && process.env.Path !== undefined ? "Path" : "PATH";
}
