...
describe("resolvePiBinary", () => {
  ...
  it("finds pi in nvm-managed node versions on unix", () => {
    const home = "/home/user";
    const piPath = `${home}/.nvm/versions/node/v24.15.0/bin/pi`;
    const result = resolvePiBinary({
      platform: "linux",
      home,
      workspaceDirs: [],
      access: mockAccess(new Set([piPath])),
      readDir: (path) => {
        expect(path).toBe(`${home}/.nvm/versions/node`);
        return ["v20.0.0", "v24.15.0"];
      },
      pathEnv: "",
    });
    expect(result).toBe(piPath);
  ...
  it("tries newer nvm-managed node versions first on unix", () => {
    const home = "/home/user";
    const newerPi = `${home}/.nvm/versions/node/v24.15.0/bin/pi`;
    const olderPi = `${home}/.nvm/versions/node/v20.0.0/bin/pi`;
    const result = resolvePiBinary({
      platform: "linux",
      home,
      workspaceDirs: [],
      access: mockAccess(new Set([newerPi, olderPi])),
      readDir: () => ["v20.0.0", "not-node", "v24.15.0"],
      pathEnv: "",
    });
    expect(result).toBe(newerPi);
  });
